...
- **個人データを扱うアクション（`getMyRecords` / `submitReflection`）は、トークンに加えて `checkStudentPassword(params)` で本人パスワードも毎回照合します。** これがないと、正規のトークンを持つ生徒が `studentName` を他人の名前に差し替えるだけで、他人の回答の閲覧・なりすまし提出ができてしまうためです（トークンはクラス共通で個人を区別しないため、トークンだけでは本人確認にならない）。教師系が毎回パスワードを送るのと同じ「セッションを持たず毎回照合」方式に揃えています。
...
認証列の凡例 … **T**: `password`（教師パスワード）必須 ／ **S**: `token`（生徒用トークン）必須 ／ **S+P**: `token` に加え `password`（本人パスワード）＋`studentName` の照合必須 ／ −: 不要
...
| `submitReflection` | S+P | `studentName, password, lessonId, text` | `created` or `updated` | 既存回答があれば上書き。本人パスワード照合あり |
...
| `getMyRecords` | S+P | `studentName, password` | `records[]` | 各recordに `theme` を含む。本人パスワード照合あり。**論理削除済み・授業が存在しない（完全削除された）回答は除外**（下記メモ参照） |
...
- **認証情報の保持**：教師は `state.teacherPassword`、生徒は `state.studentPassword` にログイン時のパスワードを保持し、以降の要認証アクション（教師系全般、生徒は `getMyRecords` / `submitReflection`）に毎回添えて送ります。GAS側がセッションを持たない毎回照合方式のためで、`handleLogout` で両方クリアします。リロードするとメモリごと消えるため再ログインになります。
