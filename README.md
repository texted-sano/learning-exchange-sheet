...
| 授業 | 授業ID / 日付 / タイトル / 発問 / 公開 / テーマ | 授業IDは `L<epoch millis>` |
...
| `teacherLogin` | T | | `className, teacherName, lessons[]` | 授業一覧を同梱（通信削減） |
...
| `getLessons` | T/S | `role`（`teacher`なら要password、他は要token） | `lessons[]` | teacherは全件、それ以外は公開済みのみ |
| `createLesson` | T | `date?, title, question, published?, theme?` | `lessonId` | |
...
| `getMyRecords` | S | `studentName` | `records[]` | 各recordに `theme` を含む |
| `getAllResponsesForStudent` | T | `studentName` | `records[]` | テーマ別モーダル用の一括取得（タブ切替はフロント側でフィルタ） |
| `getThemeEvaluationSummary` | T | `studentName` | `summary[]`（`{theme, average}`） | 使用中テーマのみ。評価なしは `average: 0` |
...
- **モーダル**：高さが変動する「テーマ」「評価まとめ」は `.modal-overlay.top-aligned` で上端固定（中央揃えだとタブ切替時に見出しが上下に踊るため）。これらのモーダル表示中はグローバルの読み込みオーバーレイを抑制し、モーダル内のインライン表示に任せます。
